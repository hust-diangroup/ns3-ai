...
TcpRlTimeBased::ConnectSocketCallbacks ()
{
  ...
  for (NodeList::Iterator i = NodeList::Begin (); i != NodeList::End (); ++i)
  {
    ...
    NS_LOG_DEBUG ("Node: " << node->GetId () << " TCP socket num: " <<
                  socketVec.GetN ());
    ...
    for (uint32_t j = 0; j < sockNum; j++)
    {
      ...
      Ptr<TcpSocketDerived> dtcpSocket = StaticCast<TcpSocketDerived>
                                         (tcpSocket);
  ...
  if (m_tcpSocket)
  {
    ...
    m_tcpSocket->TraceConnectWithoutContext ("Tx",
        MakeCallback (&TcpRlEnv::TxPktTrace, env));
    m_tcpSocket->TraceConnectWithoutContext ("Rx",
        MakeCallback (&TcpRlEnv::RxPktTrace, env));
...
uint32_t
TcpRlTimeBased::GetSsThresh (Ptr<const TcpSocketState> state,
...
void
TcpRlTimeBased::IncreaseWindow (Ptr<TcpSocketState> tcb,
...
void
TcpRlTimeBased::PktsAcked (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked,
                           const Time &rtt)
{
  NS_LOG_FUNCTION (this);
...
void
TcpRlTimeBased::CwndEvent (Ptr<TcpSocketState> tcb,
