...
class NS3AIRL
{
  public:
    ...
    void get_act();
    bool is_finished();
    std::vector<EnvType> m_local_env;
    std::vector<ActType> m_local_act;

  private:
    typedef boost::interprocess::
        allocator<EnvType, boost::interprocess::managed_shared_memory::segment_manager>
            ShmemEnvAllocator;
    typedef boost::interprocess::vector<EnvType, ShmemEnvAllocator> MyEnvVector;
    typedef boost::interprocess::
        allocator<ActType, boost::interprocess::managed_shared_memory::segment_manager>
            ShmemActAllocator;
...
template <typename EnvType, typename ActType>
void
NS3AIRL<EnvType, ActType>::get_finished()
{
...
template <typename EnvType, typename ActType>
void
NS3AIRL<EnvType, ActType>::get_act()
{
...
template <typename EnvType, typename ActType>
void
NS3AIRL<EnvType, ActType>::set_act()
{
    using namespace boost::interprocess;
    scoped_lock<interprocess_mutex> act_lock(m_lockable->act_mutex);
    if (m_lockable->act_in)
...
template <typename EnvType, typename ActType>
void
NS3AIRL<EnvType, ActType>::get_env()
{
    using namespace boost::interprocess;
...
template <typename EnvType, typename ActType>
void
NS3AIRL<EnvType, ActType>::set_env()
{
    using namespace boost::interprocess;
    scoped_lock<interprocess_mutex> env_lock(m_lockable->env_mutex);
    if (m_lockable->env_in)
